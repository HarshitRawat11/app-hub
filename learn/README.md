...
| 31 | [31-a-spend-guardrail-that-runs-in-aws.md](31-a-spend-guardrail-that-runs-in-aws.md) | *Delegated short note* — `aws_budgets_budget` in the **persistent** stack, because a guardrail destroyed nightly is absent on the nights it is needed; **alerts fire once per budget PERIOD**, which is why a second *daily* budget adds something a monthly one cannot; backstop vs watchdog (billing data updates ~daily); why there is deliberately no cost filter |
| 32 | [32-a-portfolio-site-on-netlify.md](32-a-portfolio-site-on-netlify.md) | *Delegated short note* — why app-hub **cannot** run on Netlify and what got deployed instead; why a permanent link to the live cluster is impossible; a **fetch shim** so the demo runs the REAL `app.js` unmodified; the polyrepo forcing a vendored copy and the byte-check that keeps it honest; **script order that fails silently**; a table that overflowed at 293px and was invisible at desktop width |
| 33 | [33-ingress-and-a-shared-alb.md](33-ingress-and-a-shared-alb.md) | **Full file — guided build** — an `Ingress` is inert without a controller; `IngressClass` and the class that silently matches nothing; **`target-type: ip` vs `instance`, and why it decides the Service type too**; a fixed role name survives rebuilds but a VPC id does not; `Running` ≠ `Ready` ≠ working; the controller writes AWS state Terraform cannot see; **the teardown order that orphans a billing ALB if reversed** |
| 34 | [34-a-second-deployment-target.md](34-a-second-deployment-target.md) | *Guided build, short note — **written, never deployed*** — why a second deployment target exists and why the two must not converge; **no `ports:` anywhere**, because the tunnel dials out; **Tailscale over Cloudflare Tunnel because a named tunnel needs a domain this project does not own**; the same env var needing a different value per target for the third time; no IRSA outside EKS; **`docker compose down -v` silently changes your URL**; Funnel is off by default and the failure looks like networking; node keys expire at 180 days; **Funnel is public and its hostname is in Certificate Transparency logs** |
| 35 | [35-argocd-and-the-reconciliation-half.md](35-argocd-and-the-reconciliation-half.md) | **Full file — guided build, WRITTEN NEVER APPLIED** — reconciliation vs deployment, and why "GitOps-shaped" was not GitOps; **the bootstrap paradox — nothing can apply the first Application**; app-of-apps and sync waves; **why the namespace needs its own Application rather than `CreateNamespace=true`** (it would silently drop `enforce: restricted`); a read-only deploy key separate from Jenkins's write key, and credentials discovered **by label** so an unlabelled Secret fails identically to a missing one; **`selfHeal` would recreate the Ingress during teardown and orphan a second ALB — `D-28` again**; an uncommitted image pin rolls back after ~3 minutes |
| 36 | [36-secrets-that-outlive-the-cluster.md](36-secrets-that-outlive-the-cluster.md) | *Delegated short note* — a Kubernetes Secret **dies with every `make down`**, so the source of truth has to live outside the cluster or the password changes nightly; **`chmod 600` under `/mnt/c` reports success and changes nothing**, which is why the store is in WSL home beside the deploy key; verifying a Secret **against its source** rather than against existence, because after a rotation "exists" and "correct" are different facts; why `jenkins-secrets` is a routine post-`make up` step, not setup |
| 37 | [37-proving-a-scheduled-job-ran.md](37-proving-a-scheduled-job-ran.md) | *Delegated short note* — **a job's own log cannot prove it ran on time**, because Task Scheduler catches up a missed trigger on wake and the line reads identically; the wake-to-run **gap** is what separates a wake timer (27s) from a catch-up (6 min); `Get-ScheduledTaskInfo` returns nothing for a task that exists while `schtasks` distinguishes *denied* from *cannot find*; the readable source is the TaskScheduler operational log, and it must be filtered by task NAME because a stale duplicate registration emits failure-shaped events into it |

*Steps 34 onward get added as we do them.*
