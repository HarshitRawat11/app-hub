...
- **Emptying an ECR repository takes more than one pass.** Added 2026-09-10. buildkit pushes a manifest *index* plus the child manifests it points at, and `list-images` only shows the index. Delete it and the children appear as newly-untagged digests that were not in the first listing. Live evidence: `app-hub/gateway` needed two passes — pass 1 deleted 1, pass 2 deleted 2. A single `batch-delete-image` therefore leaves the repository non-empty and `terraform destroy` fails afterwards. **Loop until `describe-images` returns `0`.**
  
  This surfaced only because the teardown *re-checked after deleting*. The first attempt stopped at `WARNING: still holds 2 image(s) — destroy would fail. Stopping rather than proceeding blindly.` Without that check it would have run on into a `destroy` that failed twenty minutes later, with the cluster still billing throughout. **The verification was worth more than the action.**
