...
| **G2** | `P-11` owner prerequisites — scoped IAM user + access key, Tailscale account/ACL/key/expiry, `.env` | **The two policy documents are now WRITTEN and validated (2026-09-22)**: `compose/iam-policy.json` — every ARN read from AWS rather than constructed, and **zero findings from IAM Access Analyzer**, with the validator first proved capable of failing — and `compose/tailscale-acl.hujson`, the complete ACL, scoping Funnel to `tag:app-hub`. **What remains is account work only**: create the user, create the access key, create the Tailscale account, paste both documents, generate a tagged reusable auth key, fill `compose/.env`. The above, working |
