...
|---|---|---|
| **G1** | `P-11` — deploy the always-on host | **RUNNING since 2026-09-25, criterion NOT yet met.** Four of five services are up and the internal path is proven end to end; the dashboard is reachable at **`http://localhost:8001`** on the host, via a deliberate **loopback-only** publish. <br><br>**Two things still stand between this and the written criterion.** (1) **n8n is not running under Compose** — the standalone container holds the name, so this is `G5`, and until then it is 4 services rather than 5. (2) **The Tailscale hostname serves nobody yet**, because the tailnet contains exactly one device — the container itself. Tailnet-only plus zero other members means no device can reach it; this was measured, not assumed, and a phone would have failed identically. **Joining any second device closes it.** <br><br>*(criterion unchanged below)* `docker compose ps` shows **5** services up — `links-service`, `gateway`, `aggregator`, `n8n`, `tailscale` — and the Tailscale hostname serves the dashboard from a non-home network. **Was "4 services" until 2026-09-20**; `G5` added `n8n` to the stack and this criterion was not updated with it, so it would have passed while a service was missing. <br><br>**Offline checks done 2026-09-20** (no cluster, no deploy): the file **parses** under `docker compose config`, resolves to exactly those five services, publishes **no ports at all**, and `n8n_data` resolves to `external: true` — the one property that, if wrong, would start n8n blank and make every stored credential permanently undecryptable. The live `n8n_data` volume exists. *Still not deployed.* |
| ~~**G2**~~ | ~~`P-11` owner prerequisites~~ | **DONE 2026-09-25.** Every criterion met and each one verified by use rather than by being told: the scoped user exists as `arn:aws:iam::314146298861:user/app-hub-compose` and its key **pulled all three images and reads DynamoDB**, while `eks`, `iam` and `ecr:DescribeImages` are all **denied** — so the scoping is proven in both directions. The Tailscale account, ACL and tagged auth key are proven by the node reporting `Tags: ['tag:app-hub']`, which cannot happen unless `tagOwners` was in the ACL **before** the key was generated. **Expiry is handled**: `KeyExpiry: None`, which is the tagged-device behaviour and closes `learn/34`'s 180-day time bomb. `.env` is complete and gitignored. | **The two policy documents are now WRITTEN and validated (2026-09-22)**: `compose/iam-policy.json` — every ARN read from AWS rather than constructed, and **zero findings from IAM Access Analyzer**, with the validator first proved capable of failing — and `compose/tailscale-acl.hujson`, the complete ACL, scoping Funnel to `tag:app-hub`. **What remains is account work only**: create the user, create the access key, create the Tailscale account, paste both documents, generate a tagged reusable auth key, fill `compose/.env`. The above, working |
...
**Gap size: 4 items** — down from 11. **`G2` closed 2026-09-25** when the owner created the scoped IAM user and the Tailscale account, and every part of it was verified by use. `G8`–`G11` closed on 2026-09-19 after the
