...
| `D-15` | Medium | [infra/.gitignore](infra/.gitignore) | `.terraform.lock.hcl` is gitignored. HashiCorp explicitly recommends committing it: with `required_providers` pinned only to `~> 5.0`, a fresh clone can resolve a *different* AWS provider 5.x than the one this project was built and tested against. That directly undercuts purpose 3 in `CLAUDE.md § 1` — "deployable by someone else" — and reproducibility is the reason the S3 backend and `uv.lock` exist at all. The fix is to stop ignoring it and commit the lock files for both stacks. Found 2026-09-09 while preparing `C-04`, which will generate its own. |
