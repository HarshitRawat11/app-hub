...
| ~~`D-25`~~ | **RESOLVED 2026-09-22 — the wake timer fired and was caught in the act.** On the night of 2026-09-20 the laptop slept at **22:02:32**, and Windows woke it at **23:30:38** — 88 minutes later, at the trigger minute, with nothing else running. Task Scheduler logged the time trigger (`id=107`) at **23:30:34**, the teardown started **23:31 IST**, finished **23:32 success (exit 0)** and logged `reported to n8n (HTTP 200)`. <br><br>**Both required signals present, from three independent sources** — the Power-Troubleshooter wake event, the Task Scheduler operational log, and the teardown's own log file. <br><br>**And it is distinguishable from a catch-up, which was the whole risk.** A wake timer fires *at* the scheduled minute; a catch-up runs whenever the machine happens to wake. `09-20`: wake `23:30:38` → run `23:31` = **~27 seconds**. `09-17` (a genuine catch-up): wake `23:34:56` → run `23:40:55` = **6 minutes**, the machine having woken for its own reasons. The gap is the evidence. <br><br>*(was: Medium — a teardown failed and the notification about it also failed)* | `scripts/scheduled-destroy.sh`, WSL DNS, n8n SMTP | **The 2026-09-14 nightly teardown exited 2, and nothing told anyone for two days.** <br><br>`logs/scheduled-destroy-2026-09-14_174528.log` ends: `Error: validating provider credentials: retrieving caller identity from STS: ... lookup sts.ap-south-1.amazonaws.com on 8.8.8.8:53: i/o timeout` → `destroy finished: failure (exit 2)`. That is the documented WSL2 DNS failure (`CLAUDE.md § 5`), here across a sleep/resume. <br><br>**Then the second layer failed too.** The script did POST to `destroy-notifier`, which ran (execution **45**) and could not send: `connect ECONNREFUSED 192.178.158.108:465`. So the failure notification failed. <br><br>**No money was lost** — the cluster had already been destroyed by hand that evening, so the teardown was a no-op that failed rather than a real teardown that was skipped. The 2026-09-16 run succeeded (`0 destroyed`, `HTTP 200`). **The 2026-09-18 run failed for an unrelated and self-inflicted reason, found 2026-09-20 by reading the log**: `Reference to undeclared resource ... aws_ecr_repository.links_service` at `jenkins-irsa.tf:99` — the ECR move to the persistent stack, made that same day, left the ephemeral stack pointing at a managed resource that was no longer in its root module. **The nightly teardown caught it before any session did.** Already fixed with three `data "aws_ecr_repository"` blocks (now referenced at lines 136–138), and confirmed behaviourally by the 2026-09-19 run exiting `0`. **That is luck, not design:** had a cluster been up, it would have billed until someone opened the logs. <br><br>**Also worth noting:** the log's own filename (`17:45:28` UTC) and its first line (`04:15 IST`) disagree by about five hours, and 04:15 IST is exactly when Windows went to sleep. The clock behaviour across suspend is **UNKNOWN** and deliberately not explained here rather than guessed at. <br><br>**CORRECTED 2026-09-18 — the claim below was too strong and is left visible rather than edited away.** It read: *"The teardown is also not running at 23:30. Both recent runs finished at ~11:18 and ~11:24 IST... the laptop is asleep at 23:30."* <br><br>**It runs at 23:30 whenever the laptop is awake.** Reading the log filenames as UTC and converting: `2026-09-16_180004` = **23:30:04 IST** and `2026-09-17_181055` = **23:40:55 IST**, both on schedule and both `exit 0`. The two late-morning runs were nights the machine was asleep, and Task Scheduler caught them up on wake. <br><br>**So the risk is CONDITIONAL, not unconditional**, and that changes what the fix is. The task itself is fine; the variable is whether the laptop is awake at 23:30. A cluster left up on a night you shut the lid still bills until morning — roughly **12 hours × $0.28 ≈ $3.40** — so this stays open. **FIX APPLIED 2026-09-18, VERIFICATION PENDING.** `-WakeToRun` is set in `register-scheduled-destroy.ps1` and the owner re-registered the task from an elevated shell. <br><br>**The second half mattered more than the first.** `-WakeToRun` is only the task's *intent*; Windows power policy decides whether wake timers are honoured at all, and it was **disabled on battery** on this machine (`DC = 0x00000000`). Setting `-WakeToRun` alone would have looked exactly like a fix and changed nothing on the nights that matter. Now measured at `AC = 0x1, DC = 0x1` — enabled on both. <br><br>**Still unverified behaviourally**, and that is the only evidence that counts here: the task settings are **unreadable unelevated** (`Get-ScheduledTask` returns nothing, `schtasks` says *Access is denied* while a made-up name says *cannot find*), so the claim rests on the elevated run reporting success. **Closes when a `logs/scheduled-destroy-*.log` appears timestamped ~23:30 IST on a night the laptop was asleep.** <br><br>**MEASURED 2026-09-20, and the defect splits cleanly in two — one half is now PROVEN, the other cannot close by waiting.** <br><br>**Half one — the notification layer works, including on failure.** Both recent nightly runs started at **23:30 IST** and both reported: `2026-09-18` **failed (exit 2)** and still logged `reported to n8n (HTTP 200)`; `2026-09-19` **succeeded (exit 0)**, also `HTTP 200`. That is precisely the scenario `D-25` was opened for — *a teardown failed and the notification about it also failed* — and the notification half now demonstrably survives a real failure, not a simulated one. <br><br>**Half two — `-WakeToRun` has still NEVER been exercised, and that is the half `G7` tracks.** Windows' own records say the laptop was **awake** at 23:30 on both nights, so neither run tested the wake timer. Reconstructed from the event log: real wakes are `Microsoft-Windows-Power-Troubleshooter` Id 1, sleeps are `Kernel-Power` Id 42 (the Id 107 events pair with a 42 seconds later and are modern-standby transitions, not resumes). That model is **corroborated independently** — it reproduces the `06:48`→`12:19` sleep already recorded under `D-24` from a different source. <br><br>slept `09-18 21:27:49` → woke `21:46:14`; slept `09-19 06:48:59` → woke `12:19:39`; slept `09-20 05:21:58` → woke `09:53:00`. **At 23:30 on both the 18th and the 19th the machine sat between a wake and the next sleep**, and there is no wake event anywhere near 23:30 on either night. The event log was confirmed readable (20 events returned) before trusting those negatives. <br><br>**THE CONSEQUENCE THAT MATTERS: `G7` will not close by waiting.** Every sleep in the record begins between **05:21 and 06:49** and ends mid-morning. On this usage pattern the laptop is *never* asleep at 23:30, so the condition can be waited on indefinitely and never met. <br><br>**It needs a deliberate test instead, and it is free:** with the cluster already down, shut the lid before 23:30 one evening. Next morning check two things together — a `logs/scheduled-destroy-*` log timestamped ~23:30 IST, **and** a Power-Troubleshooter wake event at ~23:30. The log alone is not enough: Task Scheduler also catches up a missed trigger on wake, which is what produced the `23:40:55` run on `09-17` and would look like success while proving nothing. |
...
| `D-31` | **Medium — a nightly trigger was silently skipped, then caught up into a sleep** | Windows Task Scheduler, `scripts/scheduled-destroy.sh`, WSL DNS | **Found 2026-09-22 while confirming `D-25`, and it is a different fault from `D-25` rather than a reopening of it.** <br><br>**The 2026-09-21 23:30 teardown never fired.** The Task Scheduler operational log holds **no event of any kind** for `\app-hub nightly teardown (harshit.rawat)` at 23:30 that night — no trigger, no start, no failure. The next event for that task is a time trigger (`id=107`) at **2026-09-22 03:37:16**, which is the missed run being caught up roughly four hours late. <br><br>**The machine was awake at the time**, which is what makes this a real defect rather than the known sleep behaviour: the power log records a sleep at `09-20 22:02:32` and the next at `09-22 03:37:22`, so it was continuously awake across `09-21 23:30`. **`-WakeToRun` is not the explanation, and neither is battery** — those were the causes considered and fixed under `D-25`. **The cause is UNKNOWN and is deliberately not guessed at here.** <br><br>**Then it got worse, and this part is reproducible rather than mysterious.** The catch-up started at `03:37:16` and the machine **suspended six seconds later** at `03:37:22`. The run resumed on wake at `10:39:40` and died at `10:48` with `exit 2`: `STS GetCallerIdentity ... lookup sts.ap-south-1.amazonaws.com on 8.8.8.8:53: i/o timeout` — **the documented WSL2 DNS-across-suspend failure** (`CLAUDE.md § 5`). So the teardown that was already four hours late then failed outright. <br><br>**Nothing was lost: no cluster was up**, verified 2026-09-22 — 0 clusters, 0 NAT gateways, 0 load balancers, 0 running instances, 0 orphan volumes. **That is luck, not design.** A cluster left up on that night would have billed from 23:30 to at least 10:48 — about **11 hours ≈ $3.10**. <br><br>**The notification layer behaved correctly throughout**, which is worth stating because it is the half `D-25` was about: the failure was reported, `HTTP 200`. <br><br>**Next step:** read the task's own history elevated (`Get-ScheduledTaskInfo`, or Task Scheduler UI → History) for 2026-09-21, which is the one source that can say why the trigger was skipped. It is unreadable unelevated — `schtasks /query` answers `Access is denied` for this task while an invented name answers `cannot find`, so the task exists and the refusal is permission, not absence. |
| `D-32` | **Low — a dead teardown task owned by the WORK domain account, and it cannot be removed without work-admin credentials** | Windows Task Scheduler, `scripts/register-scheduled-destroy.ps1` | **Root cause identified 2026-09-22, and it is not carelessness — it is a documented workaround leaving a permanent artifact.** <br><br>**The bare task `\app-hub nightly teardown` is registered to run as `UZIO\uzio.admin`**, read verbatim from the event text: *"Task Scheduler did not launch task \"\\app-hub nightly teardown\" because user \"UZIO\uzio.admin\" was not logged on when the launching conditions were met."* That account never logs in, so the task emits `id=332` at **every** trigger and has never run. <br><br>**This explains three observations at once**, which is why it is worth writing down rather than just deleting: it is **absent from `Get-ScheduledTask`** enumeration (another account's task), `schtasks /query` answers **`Access is denied`** rather than `cannot find` (exists, unreadable), and it fires `332` on the same schedule as the real task — so its failure-shaped events sit beside the real task's successes in one log. **That cost the first twenty minutes of the `D-25` investigation**, and is the whole reason this row exists. <br><br>**`register-scheduled-destroy.ps1` predicted this exactly.** Its `$TaskName` default **is** the bare name, and lines 90–94 warn that if a task of that name is owned by another account, registration fails with `0x800700b7` — *"a PERMISSIONS error wearing a filesystem error's clothes"* — and tell you to re-register as `app-hub nightly teardown ($env:USERNAME)`. **That guidance was followed**, which is why the working task carries the suffix. The bare one is what was left behind, presumably from an early elevated run that authenticated as the work admin. <br><br>**REMOVAL ATTEMPTED 2026-09-22 AND REFUSED**, as expected: `schtasks /delete /TN "app-hub nightly teardown" /F` → `ERROR: Access is denied`, unelevated. **The working task was verified still present immediately afterwards.** <br><br>**Owner-only, and the reason is a hard stop rather than a preference:** removing it needs an elevated shell, and elevating on this work-managed laptop authenticates as `UZIO\uzio.admin` — entering those credentials is not something to hand to an agent. From an elevated PowerShell: <br>`schtasks /delete /TN "app-hub nightly teardown" /F` <br><br>**Delete the BARE name only.** `app-hub nightly teardown (harshit.rawat)` is the working task and must survive — the two differ by a suffix, which is the same near-identical-names trap that deleted the ArgoCD deploy key on 2026-09-20. <br><br>**Doing nothing is a legitimate choice.** It costs no money and runs nothing; the only cost is the confusing `332` events, and those are now explained here.
...
turned out to be unnecessary — the conditions occurred on their own two nights
ago, and the evidence was sitting in the logs:

| source | 2026-09-20 |
|---|---|
| Kernel-Power | `SLEEP 22:02:32` |
| Power-Troubleshooter | `WAKE 23:30:38` |
| Task Scheduler | time trigger `id=107` at `23:30:34`, completed `23:32:09` |
| teardown log | started `23:31 IST`, `success (exit 0)`, `reported to n8n (HTTP 200)` |

**Three independent sources, and the laptop had been asleep for 88 minutes.**
That is exactly `D-25`'s closing condition.

**The wake-timer/catch-up distinction is what makes it proof rather than a
coincidence**, and it is measurable: a wake timer fires *at* the scheduled
minute, a catch-up fires whenever the machine happens to wake. `09-20` was
**27 seconds** from wake to run. `09-17`, a genuine catch-up, was **6 minutes**.
Without that gap the same log line would have proved nothing — which is why `G7`
required two signals rather than one.

**`D-31` — the 2026-09-21 teardown never fired, and the machine was awake.**
Found while confirming the above. No Task Scheduler event of any kind for the
teardown at `09-21 23:30`; the next is a trigger at `09-22 03:37:16`, four hours
late. The power log shows sleeps at `09-20 22:02:32` and `09-22 03:37:22` and
nothing between, so it was awake throughout. **`-WakeToRun` and battery policy
are both ruled out** — those were `D-25`'s causes and both are now proven
working. **The cause is UNKNOWN and is left that way rather than guessed at.**

**It then compounded.** The catch-up started at `03:37:16`; the machine suspended
at `03:37:22`, six seconds later. It resumed on wake and failed at `10:48` with
`exit 2` — STS lookup `i/o timeout`, the documented WSL2 DNS-across-suspend
failure. **Nothing was billing** (verified: 0 clusters, 0 NAT gateways, 0 LBs,
0 instances, 0 orphan volumes), but a cluster up that night would have run
~11 hours ≈ **$3.10**.

**`D-32` — a stale duplicate task.** Two registrations exist: the working
`app-hub nightly teardown (harshit.rawat)` and a bare `app-hub nightly teardown`
that appears only as `id=332` ("will not be run"). It never runs, so it costs
nothing — but **its failure-shaped events sit in the same log as the real
task's**, and they initially read as the real task being blocked. That cost
diagnosis time today, which is the whole reason it is written down.

**The § 9 control technique earned its place twice today.** `schtasks /query`
answers `Access is denied` for the teardown task and `cannot find` for an
invented name — different answers, so the task exists and the refusal is
permission rather than absence. And `Get-ScheduledTaskInfo` returned **nothing
for both**, which is exactly the confidently-wrong answer § 9 warns about.

**On the v1 test:** it names `D-24` and `D-25`, and **both are now closed**.
`D-31` and `D-32` are new and not part of that written test. Whether they should
gate v1 is the owner's call, not something to decide by editing the test.
