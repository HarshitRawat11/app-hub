...
| `D-32` | **Low — a stale duplicate of the teardown task is registered and never runs** | Windows Task Scheduler | **Found 2026-09-22.** There are **two** registrations: `\app-hub nightly teardown (harshit.rawat)`, which is the working one, and a bare `\app-hub nightly teardown`, which appears in the operational log **only** as `id=332` — the "will not be run" event — at `09-20 23:30:48`, `09-21 10:41:22` and `09-22 03:37:18`. <br><br>`schtasks /query` confirms the bare name exists (`Access is denied`, versus `cannot find` for an invented control), so it is registered rather than a logging artifact. `register-scheduled-destroy.ps1` names the task with a `($env:USERNAME)` suffix, so the bare one is an **older registration left behind**. <br><br>**Harmless to cost — it never runs.** But it cost diagnosis time today: its `332` events sit in the same log as the real task's and initially read as the real task being blocked. **A dead duplicate that emits failure-shaped events is worse than no duplicate**, which is why this is written down rather than ignored. <br><br>**Fix, owner-only (needs an elevated shell):** `Unregister-ScheduledTask -TaskName "app-hub nightly teardown" -Confirm:$false` — note the bare name, **not** the suffixed one, which must survive. |
